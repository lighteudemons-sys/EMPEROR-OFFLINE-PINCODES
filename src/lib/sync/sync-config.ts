...
class SyncConfigService {
  ...
  async initialize(branchId: string): Promise<SyncConfig> {
    ...
    // Load from IndexedDB
    try {
      const stored = await storage.getJSON<any>(this.storageKey);
      if (stored) {
        try {
          this.config = {
            ...stored,
            entityStates: new Map(Object.entries(stored.entityStates)),
          };
        } catch (error) {
      ...
      console.error('[SyncConfigService] Failed to access IndexedDB:', error);
    }
  ...
  private async saveToStorage(): Promise<void> {
    ...
    try {
      ...
      await storage.setJSON(this.storageKey, serialized);
    } catch (error) {
