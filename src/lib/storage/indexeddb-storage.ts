...
class IndexedDBStorage {
  ...
  async init(): Promise<void> {
    ...
    this.initPromise = (async () => {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        // Timeout to prevent hanging in restricted environments (headless browsers, etc.)
        const timeoutId = setTimeout(() => {
          console.warn('[IndexedDBStorage] Database open timed out after 5s - continuing without IndexedDB');
          resolve();
        }, 5000);
  ...
  async batchPut(storeName: string, items: any[]): Promise<void> {
    ...
    // Handle empty arrays - immediately resolve
    if (!items || items.length === 0) {
      console.log(`[IndexedDBStorage] batchPut: No items to save for ${storeName}`);
      ...
      items.forEach(item => {
        ...
        request.onsuccess = () => {
          ...
          if (completed === items.length && !errorOccurred) {
            console.log(`[IndexedDBStorage] batchPut: Saved ${completed} items to ${storeName}`);
            resolve();
          }
        };
        request.onerror = () => {
  ...
  async saveBusinessDay(businessDay: any): Promise<void> {
    try {
      await this.put('business_days', businessDay);
      console.log('[IndexedDBStorage] Saved business day:', businessDay.id);
    } catch (error) {
      console.error('[IndexedDBStorage] Error saving business day:', error);
    }
  }

  async getBusinessDays(): Promise<any[]> {
    try {
      return await this.getAll('business_days');
    } catch (error) {
      console.error('[IndexedDBStorage] Error getting business days:', error);
      return [];
    }
  }

  async getBusinessDay(id: string): Promise<any | null> {
    try {
      return await this.get('business_days', id);
    } catch (error) {
      console.error('[IndexedDBStorage] Error getting business day:', error);
      return null;
    }
  }

  async deleteBusinessDay(id: string): Promise<void> {
    try {
  ...
  // ============================================
  // SIMPLE KEY-VALUE STORAGE (localStorage replacement)
  // ============================================

  /**
   * Get a simple value by key (replaces localStorage.getItem)
   */
  async getSetting(key: string): Promise<any> {
    ...
    try {
      ...
      if (result && typeof result === 'object' && 'value' in result) {
        // If stored as {value: ...}, return the value
        return result.value;
      }
      return result; // Return as-is for backward compatibility
    } catch (error) {
      console.error(`[IndexedDBStorage] Error getting setting '${key}':`, error);
      return null;
    }
  }

  /**
   * Set a simple value by key (replaces localStorage.setItem)
   */
  async setSetting(key: string, value: any): Promise<void> {
    await this.init();
    try {
      await this.put('settings', { id: key, value, timestamp: Date.now() });
    } catch (error) {
      console.error(`[IndexedDBStorage] Error setting '${key}':`, error);
      throw error;
    }
  }

  /**
   * Remove a setting by key (replaces localStorage.removeItem)
   */
  async removeSetting(key: string): Promise<void> {
    await this.init();
    try {
      await this.delete('settings', key);
    } catch (error) {
      console.error(`[IndexedDBStorage] Error removing setting '${key}':`, error);
    }
  }

  /**
   * Get string value (helper for localStorage compatibility)
   */
  ...
  /**
   * Set string value (helper for localStorage compatibility)
   */
  ...
  /**
   * Get JSON-parsed value (helper for localStorage compatibility)
   */
  async getJSON<T = any>(key: string): Promise<T | null> {
    ...
    // If already an object, return as-is; otherwise try to parse
  ...
  /**
   * Set JSON value (helper for localStorage compatibility)
   */
  ...
  async clearSettings(): Promise<void> {
    ...
    console.log('[IndexedDBStorage] All settings cleared');
  }
}
