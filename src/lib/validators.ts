...
export const orderCreateSchema = z.object({
  ...
  manualDiscountPercent: z.number().min(0).max(100).optional(), // Manual discount percentage
  manualDiscountAmount: z.number().min(0).optional(), // Manual discount amount
  manualDiscountComment: z.string().max(200).optional(), // Reason for manual discount
  orderNumber: z.number().int().positive().optional(),
  cardReferenceNumber: z.string().max(100).nullable().optional(), // Card transaction reference number
  paymentMethodDetail: z.enum(['CARD', 'INSTAPAY', 'MOBILE_WALLET', 'TALABAT']).nullable().optional() // Card payment detail type
...
export const userCreateSchema = z.object({
  ...
  canViewFinancials: z.boolean().optional() // Permission to view monetary numbers
})
...
export const loginSchema = z.object({
  ...
  userCode: z.string().length(4).regex(/^\d{4}$/).optional(),
  pin: z.string().min(4).max(6).regex(/^\d+$/).optional(),
}).refine((data) => {
  // Must have either username+password or userCode+(password or PIN)
  const hasUsernamePassword = data.username && data.password;
  const hasUserCodePassword = data.userCode && data.password;
  const hasUserCodePin = data.userCode && data.pin;

  return hasUsernamePassword || hasUserCodePassword || hasUserCodePin;
}, {
  message: "Must provide username+password or userCode+password or userCode+PIN",
  path: ["username"]
})
