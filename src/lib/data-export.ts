...
export function exportOrders(orders: any[], format: 'csv' | 'excel' | 'pdf' = 'csv'): void {
  const exportData: ExportData = {
    ...
    data: orders.map(order => ({
      ...
      'Payment Method': order.paymentMethodDetail === 'INSTAPAY' ? 'InstaPay' : order.paymentMethodDetail === 'MOBILE_WALLET' ? 'Mobile Wallet' : order.paymentMethodDetail === 'TALABAT' ? 'Talabat' : order.paymentMethod,
