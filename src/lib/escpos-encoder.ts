...
export function generateReceiptESCPOS(data: ReceiptData): Uint8Array {
  ...
  // Manual Discount
  if ((data.manualDiscountPercent !== undefined && data.manualDiscountPercent > 0) ||
      (data.manualDiscountAmount !== undefined && data.manualDiscountAmount > 0)) {
    const discountLabel = data.manualDiscountPercent > 0
      ? `Manual Discount (${data.manualDiscountPercent}%):`
      : 'Manual Discount:';
    encoder.text(discountLabel)
      .align('right')
      .text(`-${formatMoney(data.manualDiscountAmount)}`)
      .align('left')
      .newLine();

    // Print discount comment if provided
    if (data.manualDiscountComment) {
      encoder
        .text(`  Discount Note: "${data.manualDiscountComment}"`)
        .newLines(1);
    }
  }
  ...
  if (data.paymentMethod === 'card') {
    ...
    } else if (data.paymentMethodDetail === 'TALABAT') {
      paymentLabel = 'Talabat';
    } else {
      paymentLabel = 'Card';
    }
  }
...
export function generateShiftClosingReceiptPaper1(data: ShiftClosingReportData): Uint8Array {
  ...
  const talabatTotal = formatMoney(data.paymentSummary.talabat);
...
export function generateDayClosingReceiptPaper1(shiftData: DayClosingShiftData, storeName: string, branchName: string, index: number, totalShifts: number): Uint8Array {
  ...
  const totalLines = [
    ...
    { label: 'TOTAL TALABAT', value: shiftData.totals.talabat },
...
// ============================================================================
// CAPTAIN RECEIPT FUNCTIONS
// ============================================================================

export interface CaptainReceiptData {
  storeName: string;
  branchName: string;
  orderNumber: number;
  date: Date;
...
/**
 * Generate Captain Receipt (simplified receipt for barista)
 * Shows only: logo, branch name, date/time, order #, and items (name + quantity)
 * This is handed to customer who gives it to the barista to prepare drinks
 */
export function generateCaptainReceipt(data: CaptainReceiptData): Uint8Array {
  const encoder = new ESCPOSEncoder();
  const baseFontSize = mapFontSize(data.fontSize);
  ...
  if (data.logoData) {
    ...
    // Default coffee emoji if no logo
    encoder.fontSize(baseFontSize).text('☕').newLines(1);
  }
  ...
  // Date and Time
  encoder.fontSize(baseFontSize)
    .text(`${data.date.toLocaleDateString()} ${data.date.toLocaleTimeString()}`)
    .newLines(2);

  // Items Header
  encoder.style({ bold: true })
    .text('PREPARATION ORDER')
    .newLine()
    .style({ bold: false })
    .hr('-')
    .newLines(1);

  // Items - Only show name and quantity
  encoder.fontSize(baseFontSize);
  data.items.forEach(item => {
    // Format: "2x Americano"
    const itemLine = `${item.quantity}x ${item.itemName}`;
    encoder.text(itemLine).newLine();

    // Show category name if available (for organization)
    if (item.categoryName) {
      encoder.text(`  (${item.categoryName})`).newLine();
    }
