...
export function SyncOperationsViewer({ open, onOpenChange }: SyncOperationsViewerProps) {
  ...
  const fetchOperations = async () => {
    ...
    try {
      ...
      if (!branchId) {
        throw new Error('Branch ID is required. Please make sure you are assigned to a branch.');
      }

      // Access IndexedDB directly on the client side
      ...
      setOperations(branchOperations);
    } catch (err: any) {
  ...
  const handleRetrySync = async () => {
    try {
      const branchId = user?.branchId;
      if (!branchId) {
        alert('Branch ID is required. Please make sure you are assigned to a branch.');
        return;
      }
      ...
      if (syncResult.success) {
        ...
        alert(syncResult.errors.join(', ') || 'Failed to trigger sync');
      }
    } catch (err) {
  ...
  const clearFailedOperations = async () => {
    ...
    try {
      ...
      alert(`Cleared ${failedOperations.length} failed operations`);
      fetchOperations();
    } catch (err) {
