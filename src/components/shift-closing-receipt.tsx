...
export function ShiftClosingReceipt({ shiftId, shiftData, open, onClose }: ShiftClosingReceiptProps) {
  ...
  useEffect(() => {
    if (open) {
      const loadData = async () => {
        console.log('[Shift Closing Receipt] Dialog opened, evaluating data source...');
        console.log('[Shift Closing Receipt] shiftId:', shiftId);
        console.log('[Shift Closing Receipt] shiftData:', shiftData);
        console.log('[Shift Closing Receipt] shiftData.id:', shiftData?.id);
        console.log('[Shift Closing Receipt] shiftData starts with temp-?:', shiftData?.id?.startsWith('temp-'));
        ...
        if (useShiftData) {
          ...
          console.error('[Shift Closing Receipt] No shiftId or valid shiftData provided!');
  ...
  const loadReceiptFromShiftData = async (shift: any) => {
    ...
    let cash = 0, card = 0, instapay = 0, wallet = 0, talabat = 0;
    if (paymentBreakdown) {
      ...
      talabat = paymentBreakdown.talabat || 0;
    }
    ...
    // Initialize orderTypeBreakdown outside try block so it's always defined
    ...
    try {
      ...
      // Handles formats: "وزن: 0.755x", "Weight: 1.008x", "وزن: EGP 50.00 (96g)", and "0.755x"
      const isCustomInputItem = (item: any): boolean => {
        ...
        // Check for pattern like "{any text}: 0.755x" (weight mode)
      ...
      setError('Failed to load shift closing data. Please try again.');
    }

    if (categoryBreakdown.length === 0) {
      console.warn('[Shift Closing Receipt] No items found in category breakdown');
    }
    ...
    const report = {
      ...
      paymentSummary: {
        ...
        talabat,
        other: 0,
        total: cash + card + instapay + wallet + talabat
      },
  ...
  const fetchOfflineShiftReport = async () => {
    ...
    shiftOrders.forEach((order: any) => {
      ...
      if (paymentMethod === 'cash') {
        ...
        } else if (detail === 'TALABAT') {
          talabat += order.totalAmount || 0;
        } else {
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-between items-center p-3 bg-rose-50 dark:bg-rose-950/20 rounded-lg border border-rose-200 dark:border-rose-900">
                            <span className="font-semibold text-sm">Total Talabat:</span>
                            <span className="font-bold text-rose-600 dark:text-rose-400">
                              {formatCurrency(data.paymentSummary.talabat)}
                            </span>
                          </div>
