                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="text-lg font-bold text-emerald-600">
                      {canSeeFinancials(user) ? formatCurrency(shifts[0].currentRevenue || shifts[0].closingRevenue || 0) : '••••'}
                    </p>
                  </div>
                  <div>
                    ...
                    <CardContent className="p-4">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                fetch(`/api/shifts/${shift.id}`, { method: 'DELETE' })
                                  .then(r => r.json())
                                  .then(d => { if (d.success) fetchShifts(); else alert(d.error || 'Delete failed'); })
                      ...
                      <div className="grid grid-cols-3 gap-2 text-sm">
                        <div>
                          ...
                          <p className="font-semibold text-slate-900">
                            {canSeeFinancials(user) ? formatCurrency(shift.closingRevenue || shift.currentRevenue || 0) : '••••'}
                          </p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-slate-900">
                            {canSeeFinancials(user) ? formatCurrency(shift.openingCash) : '••••'}
                          </p>
                        </div>
                      </div>
                    </CardContent>
              ...
              <Card className="bg-slate-50">
                <CardContent className="p-4 space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(selectedShift.openingCash) : '••••'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">Revenue</span>
                    <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(selectedShift.currentRevenue || selectedShift.closingRevenue || 0) : '••••'}</span>
                  </div>
                  <Separator />
                  <div className="flex justify-between font-bold">
                    <span>Expected Cash</span>
                    <span className="text-emerald-600">
                      {canSeeFinancials(user) ? formatCurrency((selectedShift.openingCash || 0) + (selectedShift.currentRevenue || selectedShift.closingRevenue || 0)) : '••••'}
                    </span>
                  </div>
                </CardContent>
              </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.paymentSummary.cash) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Card</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.paymentSummary.card) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Other</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.paymentSummary.other) : '••••'}</span>
                    </div>
                    <Separator className="my-2" />
                    <div className="flex justify-between font-bold">
                      <span>Total</span>
                      <span className="text-emerald-600">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.paymentSummary.total) : '••••'}</span>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.orderTypeBreakdown['dine-in'].total) : '••••'} ({shiftDetailsData.orderTypeBreakdown['dine-in'].count})</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Take-Away</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.orderTypeBreakdown['take-away'].total) : '••••'} ({shiftDetailsData.orderTypeBreakdown['take-away'].count})</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Delivery</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.orderTypeBreakdown['delivery'].total) : '••••'} ({shiftDetailsData.orderTypeBreakdown['delivery'].count})</span>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.sales) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Discounts</span>
                      <span className="font-medium text-red-600">-{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.discounts) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Delivery Fees</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.deliveryFees) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Refunds</span>
                      <span className="font-medium text-red-600">-{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.refunds) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Daily Expenses</span>
                      <span className="font-medium text-red-600">-{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.dailyExpenses) : '••••'}</span>
                    </div>
                    <Separator className="my-2" />
                    <div className="flex justify-between">
                      <span className="text-slate-600">Opening Cash</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.openingCashBalance) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Expected Cash</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.expectedCash) : '••••'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Closing Cash</span>
                      <span className="font-medium">{canSeeFinancials(user) ? formatCurrency(shiftDetailsData.totals.closingCashBalance) : '••••'}</span>
                    </div>
                    <Separator className="my-2" />
                    <div className="flex justify-between font-bold">
                      ...
                      <span className={shiftDetailsData.totals.overShort && shiftDetailsData.totals.overShort < 0 ? 'text-red-600' : shiftDetailsData.totals.overShort && shiftDetailsData.totals.overShort > 0 ? 'text-emerald-600' : 'text-slate-600'}>
                        {canSeeFinancials(user) ? (shiftDetailsData.totals.overShort !== null ? formatCurrency(shiftDetailsData.totals.overShort) : 'N/A') : '••••'}
                      </span>
                    </div>
                  </CardContent>
                </Card>
                        ...
                        {shiftDetailsData.voidedItems.map((item, idx) => (
                          <div key={idx} className="p-2 bg-slate-50 rounded text-xs">
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="text-red-600">-{canSeeFinancials(user) ? formatCurrency(item.voidedSubtotal) : '••••'}</span>
                            </div>
                        ...
                        {shiftDetailsData.refundedOrders.map((order, idx) => (
                          <div key={idx} className="p-2 bg-slate-50 rounded text-xs">
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="text-red-600">-{canSeeFinancials(user) ? formatCurrency(order.refundAmount) : '••••'}</span>
                            </div>
                        ...
                        {shiftDetailsData.categoryBreakdown.map((cat) => (
                          <div key={cat.categoryName}>
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <span className="text-sm font-bold text-emerald-600">{canSeeFinancials(user) ? formatCurrency(cat.totalSales) : '••••'}</span>
                            </div>
                            <div className="space-y-1 pl-2">
                              {cat.items.map((item, idx) => (
                                <div key={idx} className="flex justify-between text-xs text-slate-600">
                                  <span>{item.itemName} x{item.quantity}</span>
                                  <span>{canSeeFinancials(user) ? formatCurrency(item.totalPrice) : '••••'}</span>
                                </div>
                              ))}
                            </div>
                          </div>
