...
import { Plus, Pencil, Trash2, Users, Shield, Search, Key, Lock, Power, PowerOff, DollarSign, Eye } from 'lucide-react';
...
export default function UserManagement() {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'ADMIN':
        return <Badge className="bg-emerald-600">{t('users.admin')}</Badge>;
      case 'BRANCH_MANAGER':
        return <Badge className="bg-blue-600">{t('users.manager')}</Badge>;
      case 'CASHIER':
        return <Badge className="bg-slate-600">{t('users.cashier')}</Badge>;
      default:
  ...
  const handlePasswordChange = async () => {
    ...
    if (passwordData.newPassword !== passwordData.confirmPassword) {
      setMessage({ type: 'error', text: t('form.password.mismatch') });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'error', text: t('msg.connection.error') });
    } finally {
  ...
  const handlePinChange = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        setMessage({ type: 'error', text: data.error || 'Failed to set PIN' });
        return;
      }
  ...
  const openPinDialog = (user: User) => {
    ...
    if (user.role === 'ADMIN') {
      setMessage({ type: 'error', text: 'PIN is only available for Cashiers and Branch Managers' });
      setTimeout(() => setMessage(null), 3000);
      return;
    }
    ...
    if (currentUser?.role === 'BRANCH_MANAGER') {
      if (user.branchId !== currentUser.branchId) {
        setMessage({ type: 'error', text: 'You can only set PIN for users in your branch' });
        setTimeout(() => setMessage(null), 3000);
        return;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submissionData: any = {
        ...
        ...(currentUser?.role === 'ADMIN' && { canViewFinancials: formData.canViewFinancials }),
      };
  ...
  return (
    <div className="space-y-6">
            ...
            {canCreateUser() && (
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>{editingUser ? t('users.edit.user') : 'Add New User'}</DialogTitle>
                  </DialogHeader>
                  <form onSubmit={handleSubmit}>
                      ...
                      {currentUser?.role === 'ADMIN' && (
                        <div className="flex items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            <Label htmlFor="canViewFinancials" className="flex items-center gap-2">
                              <Eye className="h-4 w-4" />
                              View Financials
                            </Label>
                            <p className="text-xs text-slate-500">
                              Allow this user to see financial data (shifts, reports, revenue).
                            </p>
                          </div>
                          <Switch
                            id="canViewFinancials"
                            checked={formData.canViewFinancials ?? false}
                            onCheckedChange={(checked) => setFormData({ ...formData, canViewFinancials: checked })}
                          />
                        </div>
                      )}
                    </div>
                    <DialogFooter className="flex-col-reverse gap-2 sm:flex-row">
                      ...
                      <Button
                        ...
                        disabled={loading}
                      >
                        {loading ? t('msg.saving') : editingUser ? t('btn.update') : t('btn.add')} User
                      </Button>
                    </DialogFooter>
                  </form>
                </DialogContent>
                        ...
                        <div className="flex justify-end gap-2">
                          {user.role !== 'ADMIN' && (
                            <Button
                              ...
                              title={user.hasPin ? 'Change PIN' : t('users.set.pin')}
                            >
      ...
      {/* {t('users.change.password')} Dialog */}
      ...
      <Dialog open={pinDialogOpen} onOpenChange={setPinDialogOpen}>
        <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-[#0F3A2E] dark:text-[#FFFDF8]">
              <Lock className="h-5 w-5 text-[#C7A35A]" />
              {pinTargetUser?.hasPin ? 'Change PIN' : t('users.set.pin')}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              {pinTargetUser?.hasPin ? t('users.change.pin.for') : 'Set PIN for: '} <strong>{pinTargetUser?.username}</strong>
              {pinTargetUser?.userCode && (
                <>
                  {' '} (Code: <span className="font-mono bg-slate-100 px-2 py-0.5 rounded">{pinTargetUser.userCode}</span>)
                </>
              )}
            </p>
            <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500 text-center">
              PIN must be 4-6 digits. Used for quick login with user code.
            </p>
          </div>
          <DialogFooter className="flex-col-reverse gap-2 sm:flex-row">
            <Button
