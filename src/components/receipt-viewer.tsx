...
interface Order {
  ...
  etaAcceptedAt?: string | null;
  etaQRCode?: string | null;
  etaDocumentType?: string | null;
  isCreditNote?: boolean;
  creditNoteReason?: string | null;
  originalOrderUUID?: string | null;
}
                ...
                {order.etaUUID && (
                  ...
                  color: '#000 !important'
                    ...
                    <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>
                      {order.isCreditNote ? 'ETA Credit Note' : 'ETA Receipt'}
                    </div>
                    <div>UUID: {order.etaUUID.substring(0, 20)}...</div>
                    {order.etaSubmissionStatus && (
                      <div style={{ fontSize: '9px', marginTop: '2px' }}>
                        Status: {order.etaSubmissionStatus}
                      </div>
                    )}
                    {order.etaAcceptedAt && (
                      <div style={{ fontSize: '9px' }}>
                        Accepted: {new Date(order.etaAcceptedAt).toLocaleString()}
                      </div>
                    )}
                    {order.etaQRCode && (
