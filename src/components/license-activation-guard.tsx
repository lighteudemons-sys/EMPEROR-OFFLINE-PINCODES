...
export function LicenseActivationGuard({ children }: LicenseActivationGuardProps) {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      // Add timeout to prevent hanging on IndexedDB issues in restricted environments
      const registration = await Promise.race([
        isDeviceRegistered(),
        new Promise<{ registered: boolean; branchId?: string }>((resolve) =>
          setTimeout(() => resolve({ registered: false }), 3000)
        ),
      ]);
