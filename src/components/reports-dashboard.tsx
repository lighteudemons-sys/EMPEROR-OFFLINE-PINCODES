...
import { getDateRangeFromPreset, type PresetKey, type DateRange } from '@/lib/date-range';
import {
  ...
  ArrowRight, Eye, Printer, RefreshCw as RefreshIcon, XCircle, X, Smartphone, Tag, ShoppingBag
} from 'lucide-react';
...
interface Order {
  ...
  paymentMethodDetail?: 'CARD' | 'INSTAPAY' | 'MOBILE_WALLET' | 'TALABAT' | null;
  orderType: string;
  isRefunded: boolean;
  refundReason?: string;
...
// Map old preset keys to new PresetKey type
const LEGACY_TO_PRESET: Record<string, PresetKey> = {
  today: 'today',
  yesterday: 'yesterday',
  week: 'thisWeek',
  lastWeek: 'lastWeek',
...
export default function ReportsDashboard() {
  ...
  const [selectedPreset, setSelectedPreset] = useState<PresetKey | 'custom'>('thisYear');
  const [activeRange, setActiveRange] = useState<DateRange>(() => getDateRangeFromPreset('thisYear'));

  const handleDateChange = (key: PresetKey | 'custom', range: DateRange) => {
    setSelectedPreset(key);
    setActiveRange(range);
  };

  // Helper used by all fetch functions — replaces 5 duplicated blocks
  const getDates = () => ({ startDate: activeRange.startDate, endDate: activeRange.endDate });
  ...
  }, [selectedBranch, activeRange, comparePeriod, activeTab]);

  // Reset page when filters change
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedBranch, activeRange]);
  ...
  }, [selectedBranch, activeRange, activeTab, currentPage]);
  ...
  const fetchOrdersByPaymentMethod = async (paymentMethod: string) => {
    ...
    try {
      ...
      if (method === 'cash') {
        ...
        // Credit Card — card method with CARD detail
        params.append('paymentMethod', 'card');
        params.append('paymentMethodDetail', 'CARD');
      } else if (method === 'credit') {
        // On-account credit
        params.append('paymentMethod', 'credit');
      } else if (method === 'instapay') {
        params.append('paymentMethod', 'card');
        params.append('paymentMethodDetail', 'INSTAPAY');
      } else if (method === 'mobile_wallet' || method === 'wallet') {
        params.append('paymentMethod', 'card');
        params.append('paymentMethodDetail', 'MOBILE_WALLET');
      } else if (method === 'talabat') {
        params.append('paymentMethod', 'card');
        params.append('paymentMethodDetail', 'TALABAT');
      }
      ...
      if (data.orders) {
        setPaymentMethodOrders(data.orders);
      } else {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Payment Methods — 5 Fixed Cards */}
            {(() => {
              const methods = kpiData?.paymentMethods || {};
              const cards = [
                { key: 'cash', label: 'Cash', Icon: DollarSign, color: 'green', bgClass: 'from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/20 border-green-200 dark:border-green-800', iconBg: 'bg-green-100 dark:bg-green-900', iconColor: 'text-green-600 dark:text-green-400' },
                { key: 'card', label: 'Credit', Icon: CreditCard, color: 'blue', bgClass: 'from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/20 border-blue-200 dark:border-blue-800', iconBg: 'bg-blue-100 dark:bg-blue-900', iconColor: 'text-blue-600 dark:text-blue-400' },
                { key: 'instapay', label: 'InstaPay', Icon: Smartphone, color: 'purple', bgClass: 'from-purple-50 to-violet-50 dark:from-purple-950/30 dark:to-violet-950/20 border-purple-200 dark:border-purple-800', iconBg: 'bg-purple-100 dark:bg-purple-900', iconColor: 'text-purple-600 dark:text-purple-400' },
                { key: 'mobile_wallet', label: 'Mobile Wallet', Icon: Wallet, color: 'orange', bgClass: 'from-orange-50 to-amber-50 dark:from-orange-950/30 dark:to-amber-950/20 border-orange-200 dark:border-orange-800', iconBg: 'bg-orange-100 dark:bg-orange-900', iconColor: 'text-orange-600 dark:text-orange-400' },
                { key: 'talabat', label: 'Talabat', Icon: ShoppingBag, color: 'rose', bgClass: 'from-rose-50 to-pink-50 dark:from-rose-950/30 dark:to-pink-950/20 border-rose-200 dark:border-rose-800', iconBg: 'bg-rose-100 dark:bg-rose-900', iconColor: 'text-rose-600 dark:text-rose-400' },
              ];

              const hasAnyData = cards.some(c => methods[c.key]?.count > 0);

              if (!hasAnyData && kpiData) {
                return (
                  <Card className={cards[0].bgClass}>
                    <CardContent className="pt-6">
                      <div className="text-center py-8 text-slate-500">
              ...
              return <div className="space-y-3">
                {cards.map(({ key, label, Icon: CardIcon, bgClass, iconBg, iconColor }) => {
                  ...
                  return (
                    <Card
                      key={key}
                      className={`cursor-pointer hover:shadow-md transition-all ${bgClass}`}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              ...
                              <p className={`text-xl font-bold tabular-nums ${count === 0 ? 'text-slate-400 dark:text-slate-600' : 'text-slate-900 dark:text-white'}`}>
                                {formatCurrency(revenue, currency)}
                              </p>
                            </div>
                            <div className="flex items-center justify-between mt-0.5">
                              <p className="text-xs text-slate-500 dark:text-slate-400">{count} {t('shifts.orders')}</p>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`p-3 rounded-xl ${
                  ...
                  selectedPaymentMethod?.toLowerCase() === 'talabat' ? 'bg-rose-100 dark:bg-rose-900/30' :
                  ...
                  {selectedPaymentMethod?.toLowerCase() === 'talabat' && <ShoppingBag className="h-6 w-6 text-rose-600 dark:text-rose-400" />}
                ...
                <div>
                  ...
                  <DialogDescription>
                    {paymentMethodOrders.length} orders • {activeRange.label}
                  </DialogDescription>
                </div>
              </div>
            </div>
