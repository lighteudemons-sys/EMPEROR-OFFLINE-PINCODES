...
import { Clock, DollarSign, ShoppingCart, Play, Square, AlertCircle, Calendar, User, TrendingUp, Store, CreditCard, Wallet, CircleDollarSign, Activity, Smartphone, X, RefreshCw, ChevronLeft, ChevronRight, Trash2, ShoppingBag } from 'lucide-react';
  ...
  try {
    ...
    await indexedDBStorage.addOperation({
      ...
      data: {
        ...shiftData,
        id: tempId,
        dayId: shiftData.dayId, // Include dayId to link shift to business day
...
async function openBusinessDayOffline(businessDayData: any, user: any): Promise<any> {
  try {
    ...
    const { getIndexedDBStorage } = await import('@/lib/storage/indexeddb-storage');
    const indexedDBStorage = getIndexedDBStorage();
    console.log('[Business Day] IndexedDB storage imported');
    await indexedDBStorage.init();
    console.log('[Business Day] IndexedDB storage initialized');
    ...
    // Save business day to IndexedDB
    await indexedDBStorage.saveBusinessDay(newBusinessDay);
    console.log('[Business Day] Business day saved to IndexedDB with ID:', tempId);

    // Verify it was saved
    const verifyBusinessDays = await indexedDBStorage.getBusinessDays();
...
async function closeShiftOffline(
  ...
  try {
    ...
    // Calculate all discounts from orders
...
export default function ShiftManagement() {
  ...
  // Pagination state for managers/admins
  const [shiftsPage, setShiftsPage] = useState(1);
  const [deletingShiftId, setDeletingShiftId] = useState<string | null>(null);
  const [shiftsTotalPages, setShiftsTotalPages] = useState(1);
  const [shiftsPerPage] = useState(user?.role === 'CASHIER' ? 5 : 5);
  ...
  const fetchBusinessDayStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        if (data.status === 'OPEN' && data.businessDay) {
          ...
          console.log('[Shift Management] API returned no open business day, checking IndexedDB');
        }
      } else {
        console.log('[Shift Management] API returned error:', response.status, 'checking IndexedDB');
      }
    } catch (error) {
    ...
    try {
      ...
      console.log('[Shift Management] All business days in IndexedDB:', businessDays);
      ...
      if (openBusinessDay) {
        console.log('[Shift Management] Found open business day in IndexedDB:', openBusinessDay);
        setBusinessDayStatus({
          isOpen: true,
          businessDayId: openBusinessDay.id,
        });
      } else {
        console.log('[Shift Management] No open business day found in IndexedDB for branch:', selectedBranch);
        setBusinessDayStatus({
          isOpen: false,
        });
      }
    ...
    const fetchDailyExpenses = async () => {
      ...
      try {
        ...
        if (response.ok && data.expenses) {
          // ALL daily expenses affect expected cash (money taken from drawer)
          // INVENTORY expenses are included because the cashier used cash to buy inventory
          const total = data.expenses.reduce((sum: number, exp: any) => sum + exp.amount, 0);
          console.log('[Shift Management] Daily expenses from API:', {
            shiftId: selectedShift.id,
            totalExpenses: data.expenses.length,
            total,
            categories: data.expenses.map((e: any) => e.category),
          });
          setCurrentDailyExpenses(total);
        } else {
          // API returned no data or error, try IndexedDB
          console.log('[Shift Management] API returned no data, checking IndexedDB');
          throw new Error('No expenses from API');
        }
      } catch (error) {
        console.error('[Shift Management] Failed to fetch daily expenses from API, checking IndexedDB:', error);
        // Try to fetch from IndexedDB for offline support
        try {
          const { getIndexedDBStorage } = await import('@/lib/storage/indexeddb-storage');
          const indexedDBStorage = getIndexedDBStorage();
          await indexedDBStorage.init();
          ...
          console.log('[Shift Management] All expenses from IndexedDB:', allExpenses.length);

          const shiftExpenses = allExpenses.filter((exp: any) => {
            console.log('[Shift Management] Checking expense:', {
              expShiftId: exp.shiftId,
              selectedShiftId: selectedShift.id,
              match: exp.shiftId === selectedShift.id,
            });
            return exp.shiftId === selectedShift.id;
          });

          console.log('[Shift Management] Shift expenses from IndexedDB:', {
            shiftId: selectedShift.id,
            expensesCount: shiftExpenses.length,
            expenses: shiftExpenses.map((e: any) => ({ id: e.id, category: e.category, amount: e.amount })),
          });

          // ALL daily expenses affect expected cash (money taken from drawer)
          const total = shiftExpenses.reduce((sum: number, exp: any) => sum + exp.amount, 0);

          console.log('[Shift Management] Daily expenses from IndexedDB:', {
            shiftId: selectedShift.id,
          ...
          console.error('[Shift Management] Failed to fetch daily expenses from IndexedDB:', dbError);
          setCurrentDailyExpenses(0);
        }
      } finally {
  ...
  const getFilteredShifts = () => {
    // For cashiers: show only last 5 shifts
    if (user?.role === 'CASHIER') {
      return [...shifts]
    ...
    // For managers and admins: show all shifts with pagination
  ...
  useEffect(() => {
    setPaymentBreakdown(prev => ({
      ...prev,
      total: prev.cash + prev.card + prev.instapay + prev.wallet + prev.talabat,
    }));
  }, [paymentBreakdown.cash, paymentBreakdown.card, paymentBreakdown.instapay, paymentBreakdown.wallet, paymentBreakdown.talabat]);

  // Update shifts page when filters change (for managers/admins)
  useEffect(() => {
    setShiftsPage(1);
  }, [selectedBranch, selectedStatus, selectedCashier]);

  // Custom fetch function for paginated shifts (managers/admins only)
  const fetchPaginatedShifts = async () => {
    if (user?.role === 'CASHIER') {
      // Cashiers use the regular shiftsData (last 5 shifts)
      return;
    }

    // Managers and admins fetch all shifts with pagination (5 per page)
    const limit = 5;
    const offset = (shiftsPage - 1) * limit;
  ...
  return (
    <div className="space-y-4 md:space-y-6 px-2 md:px-0 pb-safe-bottom">
            ...
            {selectedShift ? (
              <>
                  ...
                  <Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-950 dark:to-green-900 border-green-200 dark:border-green-800">
                    <CardContent className="pt-4 md:pt-6">
                      ...
                      <p className="text-xl md:text-2xl font-bold text-green-900 dark:text-green-100">
                        {canSeeFinancials(user) ? formatCurrency(
                  ...
                  <Card className="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-950 dark:to-amber-900 border-amber-200 dark:border-amber-800">
                    <CardContent className="pt-4 md:pt-6">
                      ...
                      <p className="text-xl md:text-2xl font-bold text-amber-900 dark:text-amber-100">
                        {canSeeFinancials(user) ? formatCurrency(selectedShift.openingCash, currency) : '••••'}
                      </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
                  ...
                  <div className="flex items-center gap-3 p-3 md:p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                    <div className="p-2 bg-rose-100 dark:bg-rose-900 rounded-lg shrink-0">
                      <ShoppingBag className="h-5 w-5 text-rose-600" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs md:text-sm text-slate-600 dark:text-slate-400">Talabat</p>
                      <p className="text-base md:text-lg font-bold truncate">
                        {canSeeFinancials(user)
                          ? (selectedShift.paymentBreakdown?.talabat !== undefined
                            ? formatCurrency(selectedShift.paymentBreakdown.talabat, currency)
                            : '—')
                          : '••••'}
                      </p>
                    </div>
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="whitespace-nowrap">{t('shift.cashier')}</TableHead>
                        ...
                        <TableHead className="text-right whitespace-nowrap">Talabat</TableHead>
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              {canSeeFinancials(user) ? formatCurrency(shift.openingCash, currency) : '••••'}
                            </TableCell>
                            <TableCell className="text-right whitespace-nowrap">
                              {shift.closingCash !== undefined ? (
                                <span className="font-medium">
                                  {canSeeFinancials(user) ? formatCurrency(shift.closingCash, currency) : '••••'}
                                </span>
                              ) : (
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              <div className="flex items-center justify-end gap-1">
                                ...
                                <span className="font-medium">
                                  {canSeeFinancials(user) ? formatCurrency(stats.revenueDuringShift, currency) : '••••'}
                                </span>
                              </div>
                            </TableCell>
                            <TableCell className="text-right whitespace-nowrap">
                              {shift.paymentBreakdown?.cash !== undefined ? (
                                <span className="text-green-600">
                                  {canSeeFinancials(user) ? formatCurrency(shift.paymentBreakdown.cash, currency) : '••••'}
                                </span>
                              ) : (
                                <span className="text-slate-400">—</span>
                              )}
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              {shift.paymentBreakdown?.card !== undefined ? (
                                <span className="text-blue-600">
                                  {canSeeFinancials(user) ? formatCurrency(shift.paymentBreakdown.card, currency) : '••••'}
                                </span>
                              ) : (
                                <span className="text-slate-400">—</span>
                              )}
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              {shift.paymentBreakdown?.instapay !== undefined ? (
                                <span className="text-purple-600">
                                  {canSeeFinancials(user) ? formatCurrency(shift.paymentBreakdown.instapay, currency) : '••••'}
                                </span>
                              ) : (
                                <span className="text-slate-400">—</span>
                              )}
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              {shift.paymentBreakdown?.wallet !== undefined ? (
                                <span className="text-orange-600">
                                  {canSeeFinancials(user) ? formatCurrency(shift.paymentBreakdown.wallet, currency) : '••••'}
                                </span>
                              ) : (
                                <span className="text-slate-400">—</span>
                              )}
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              {shift.paymentBreakdown?.talabat !== undefined ? (
                                <span className="text-rose-600">
                                  {canSeeFinancials(user) ? formatCurrency(shift.paymentBreakdown.talabat, currency) : '••••'}
                                </span>
                              ) : (
                                <span className="text-slate-400">—</span>
                              )}
                            ...
                            <TableCell className="text-right whitespacenowrap">
                              {user?.role === 'ADMIN' && (
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className="h-9 min-h-[36px] text-red-600 hover:text-red-700 hover:bg-red-50 mr-1"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    if (!confirm('Delete this shift and ALL linked orders, expenses, and costs? This cannot be undone.')) return;
                                    setDeletingShiftId(shift.id);
                                    fetch(`/api/shifts/${shift.id}`, { method: 'DELETE' })
                                      .then(r => r.json())
                                      .then(d => { if (d.success) refetchShifts(); else alert(d.error || 'Delete failed'); })
      ...
      {/* Pagination for managers and admins */}
      {user?.role !== 'CASHIER' && shifts && shifts.length > 0 && (
        <div className="flex items-center justify-between mt-4 px-4">
          <div className="text-sm text-slate-600">
            Showing {Math.min(shiftsPage * 5, shifts.length)} of {shifts.length} shifts
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShiftsPage(prev => Math.max(1, prev - 1))}
              disabled={shiftsPage === 1}
            >
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <span className="px-2 text-sm font-medium">{shiftsPage}</span>
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShiftsPage(prev => prev + 1)}
              disabled={shiftsPage >= Math.ceil(shifts.length / 5)}
            >
          ...
          {selectedShift && (
            <>
              <div className="space-y-4 py-2">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4 p-3 md:p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                  ...
                  <div>
                    ...
                    <div className="text-base md:text-lg font-bold text-green-600">
                      {canSeeFinancials(user) ? formatCurrency(getShiftStats(selectedShift).revenueDuringShift, currency) : '••••'}
                    </div>
                  </div>
                  <div>
                ...
                {/* Payment Breakdown - Hidden from non-admin */}
                {canSeeFinancials(user) && (
                <div className="p-3 md:p-4 bg-blue-50 dark:bg-blue-950 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 md:gap-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="talabatPayments" className="text-xs">Talabat</Label>
                      <Input
                        id="talabatPayments"
                        type="number"
                        step="0.01"
                        value={paymentBreakdown.talabat}
                        onChange={(e) => setPaymentBreakdown(prev => ({ ...prev, talabat: parseFloat(e.target.value) || 0 }))}
                        placeholder="0.00"
                        className="h-11 text-base"
                      />
                    </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-800">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-blue-700 dark:text-blue-300">
                        {formatCurrency(paymentBreakdown.cash + paymentBreakdown.card + paymentBreakdown.instapay + paymentBreakdown.wallet + paymentBreakdown.talabat, currency)}
                      </span>
                    </div>
                  </div>
                ...
                {/* Live Discrepancy Detection - Admin only */}
                {canSeeFinancials(user) && closingCash && (
                  <div
