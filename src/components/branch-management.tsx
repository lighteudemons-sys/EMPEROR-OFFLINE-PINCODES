...
import { Plus, Pencil, Trash2, LayoutDashboard, Key, AlertTriangle, CheckCircle, Clock, Search, Settings, Phone, MapPin, Shield, RefreshCw, Mail } from 'lucide-react';
...
export default function BranchManagement() {
    ...
    try {
      ...
      if (response.ok && data.branches) {
        const branchesList = data.branches.map((branch: any) => ({
          ...
          shiftReportEmail: branch.shiftReportEmail || undefined,
      ...
      if (editingBranch) {
        ...
        const response = await fetch('/api/branches', {
          ...
          body: JSON.stringify({
            ...
            shiftReportEmail: formData.shiftReportEmail,
          }),
        });
  ...
  const handleEdit = (branch: Branch) => {
    ...
    setFormData({
      ...
      shiftReportEmail: branch.shiftReportEmail || '',
    });
    setDialogOpen(true);
  };
                  ...
                  <DialogContent className="sm:max-w-[500px]">
                    <form onSubmit={handleSubmit}>
                      ...
                      <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="shiftReportEmail" className="flex items-center gap-1.5">
                          ...
                          <div className="relative">
                            ...
                            <Input
                              id="shiftReportEmail"
                              type="email"
                              value={formData.shiftReportEmail}
                              onChange={(e) => setFormData({ ...formData, shiftReportEmail: e.target.value })}
                              placeholder="manager@branch.com"
                              className="pl-10"
                            />
                          </div>
                          <p className="text-xs text-slate-500">Shift closing reports will be sent to this email automatically</p>
                        </div>
                        <div className="space-y-2">
                  ...
                  <Table className="w-full table-fixed">
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="w-[200px]">Report Email</TableHead>
                        ...
                        return (
                          <TableRow key={branch.id}>
                            ...
                            <TableCell className="w-[200px]">
                              {branch.shiftReportEmail ? (
                                <span className="text-sm text-slate-600 truncate block" title={branch.shiftReportEmail}>{branch.shiftReportEmail}</span>
                              ) : (
