 ...
 * - limit: Optional - Number of shifts to return (default: 100 for cashiers, all for managers/admins)
 * - offset: Optional - Number of shifts to skip (for pagination)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Calculate current revenue for open shifts and actual revenue (excluding refunds) for closed shifts
    const shiftsWithRevenue = await Promise.all(
      shifts.map(async (shift) => {
        // For both open and closed shifts, calculate actual revenue from orders excluding refunds
        // Revenue = subtotal (excludes delivery fees which go to couriers, excludes refunded orders)
        const orderStats = await db.order.aggregate({
          where: {
            shiftId: shift.id,
            isRefunded: false, // Exclude refunded orders
          },
          _sum: {
            subtotal: true,  // Revenue = subtotal (no delivery fees, no refunds)
          },
          _count: true,
        });

        const actualRevenue = orderStats._sum.subtotal || 0;
        const orderCount = orderStats._count;

        // For closed shifts, also get loyalty discounts for accurate reporting
        ...
        if (shift.isClosed) {
          ...
          loyaltyDiscounts = loyaltyDiscountStats._sum.amount || 0;
        }
        ...
        // Payment breakdown also excludes delivery fees and refunded orders
        ...
        orderPaymentStats.forEach(stat => {
          ...
          if (method === 'cash') {
            ...
            if (detail === 'INSTAPAY') {
              ...
              paymentBreakdown.talabat = (paymentBreakdown.talabat || 0) + (stat._sum.subtotal || 0);
            } else {
        ...
        return {
          ...
          // For closed shifts, override closingRevenue with actual revenue excluding refunds
          closingRevenue: shift.isClosed ? actualRevenue : shift.closingRevenue,
          currentRevenue: !shift.isClosed ? actualRevenue : shift.currentRevenue,
          currentOrders: !shift.isClosed ? orderCount : shift.currentOrders,
