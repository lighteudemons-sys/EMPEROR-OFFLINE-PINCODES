...
export async function POST(request: NextRequest) {
  try {
    ...
    const { branchId, amount, description, createdBy, transactionDate } = body;
    ...
    // Parse transactionDate if provided (YYYY-MM-DD)
    let parsedTransactionDate: Date | undefined;
    if (transactionDate) {
      parsedTransactionDate = new Date(transactionDate + 'T12:00:00'); // noon to avoid timezone issues
    }
