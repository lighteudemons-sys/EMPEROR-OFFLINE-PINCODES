...
async function createShift(data: any, branchId: string): Promise<void> {
  ...
  if (data.startTime && data.id && data.id.startsWith('temp-')) {
    ...
    const unlinkedOrders = await db.order.findMany({
      where: {
        ...
        OR: [
          { shiftId: null },
          { shiftId: data.id }, // Orders with this temp shiftId
          // Also check for orders that might have been linked to a wrong shift (from a different time)
          { shiftId: { not: null } } // We'll filter by time below
        ],
        orderTimestamp: {
          gte: shiftStartTime,
          lte: shiftEndTime,
    ...
    // Filter out orders that are already correctly linked to this shift (if it existed before)
    // and keep only unlinked ones or ones with the temp shift ID
    const ordersToLink = unlinkedOrders.filter(o => o.shiftId === null || o.shiftId === data.id);

    if (ordersToLink.length > 0) {
      addLog(`[BatchPush] Found ${ordersToLink.length} unlinked orders created before this shift, linking them now...`);
      addLog(`[BatchPush] Orders to link:`, ordersToLink.map(o => ({ number: o.orderNumber, currentShiftId: o.shiftId })));

      await db.order.updateMany({
        where: {
          id: { in: ordersToLink.map(o => o.id) },
        },
        data: { shiftId: newShift.id },
      });

      addLog(`[BatchPush] Linked ${ordersToLink.length} orders to shift ${newShift.id}:`, ordersToLink.map(o => o.orderNumber));
    } else {
      addLog(`[BatchPush] No unlinked orders found for this shift`);
      addLog(`[BatchPush] All orders in time window already linked:`, unlinkedOrders.map(o => ({ number: o.orderNumber, shiftId: o.shiftId })));
    }

    // CRITICAL: Also link any expenses with temp shift ID or null shiftId
    const unlinkedExpenses = await db.dailyExpense.findMany({
      where: {
        branchId,
        OR: [
    ...
    if (unlinkedExpenses.length > 0) {
      addLog(`[BatchPush] Found ${unlinkedExpenses.length} unlinked expenses for this shift, linking them now...`);
      await db.dailyExpense.updateMany({
        where: {
          id: { in: unlinkedExpenses.map(e => e.id) },
        },
      ...
      addLog(`[BatchPush] Linked ${unlinkedExpenses.length} expenses to shift ${newShift.id}`);
    } else {
      addLog(`[BatchPush] No unlinked expenses found for this shift`);
    }
  }
}
...
async function createOrUpdateDailyExpenseCost(
  ...
  const iconName = iconMap[category] || 'DollarSign';

  // Get or create cost category with the actual category name
  ...
  if (!costCategory) {
    ...
    console.log('[Sync] Created cost category:', costCategory.id, costCategory.name);
  }
  ...
  if (existingCost) {
    ...
    console.log('[Sync] Updated existing cost:', branchCost.id, 'Category:', costCategory.name, 'New total:', newAmount);
  } else {
    ...
    console.log('[Sync] Created new cost:', branchCost.id, 'Category:', costCategory.name);
  }
...
async function createDailyExpense(data: any, branchId: string): Promise<void> {
  ...
  // CRITICAL: Log the quantity for inventory expenses
  if (data.category === 'INVENTORY') {
    console.log('[Sync] INVENTORY expense - quantity:', data.quantity, 'type:', typeof data.quantity);
  }
  ...
  if (data.category === 'INVENTORY' && data.ingredientId && data.quantity) {
    console.log('[Sync] Processing INVENTORY expense:', {
      ingredientId: data.ingredientId,
      quantity: data.quantity,
      unitPrice: data.unitPrice,
      inventoryAlreadyUpdated: data._offlineData?.inventoryAlreadyUpdated
    });
    ...
    if (inventoryAlreadyUpdated && data._offlineData?.finalStock !== undefined && data._offlineData?.finalPrice !== undefined) {
      ...
      console.log('[Sync] Creating inventory transaction:', {
      ...
      if (shouldUseOfflineFinalState) {
        ...
        // Create inventory transaction record for audit trail
        console.log('[Sync] Creating inventory transaction (offline final state):', {
          transactionType: 'RESTOCK',
          quantityChange: data.quantity,
          stockBefore: oldStock,
          stockAfter: finalStock
        ...
        // Create inventory transaction record
        console.log('[Sync] Creating inventory transaction (normal):', {
          transactionType: 'RESTOCK',
          quantityChange: data.quantity,
          stockBefore: oldStock,
          stockAfter: newStock
