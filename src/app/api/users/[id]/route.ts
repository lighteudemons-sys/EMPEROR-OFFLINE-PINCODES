...
export async function PATCH(
  ...
  try {
    ...
    const { username, email, name, role, branchId, requesterId, requesterRole, isActive, userCode, dailyRate, canViewFinancials } = body;
    ...
    // Check userCode uniqueness if being changed
    if (userCode && userCode !== userToUpdate.userCode) {
      ...
      if (existingUserCode) {
        return NextResponse.json(
          { success: false, error: 'User code already exists' },
          { status: 400 }
        );
      }
    }
    ...
    if (canViewFinancials !== undefined) updateData.canViewFinancials = canViewFinancials;
