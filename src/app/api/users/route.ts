...
// Helper function to generate next available 4-digit user code
async function generateNextUserCode(): Promise<string> {
  // Start from 1001
  const startCode = 1001;
  const endCode = 9999;

  // Find all existing user codes
  ...
  throw new Error('No available user codes. Please contact administrator.');
}
  ...
  try {
    ...
    const usersWithBranchNames = await Promise.all(
      users.map(async (user) => {
        ...
        return { ...user, branchName: branch?.branchName || null, hasPin: !!user.pin };
      })
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { username, email, password, name, role, branchId, userCode: providedUserCode, dailyRate, canViewFinancials } = validation.data!;
    ...
    // Generate user code for CASHIER and BRANCH_MANAGER roles
    // Use provided userCode if valid, otherwise auto-generate
    let userCode: string | undefined;
    if (role === 'CASHIER' || role === 'BRANCH_MANAGER') {
      // If a custom userCode is provided and valid, use it
      if (providedUserCode && /^\d{4}$/.test(providedUserCode)) {
        // Check if the code is already taken
        const existingUserWithCode = await db.user.findUnique({
          where: { userCode: providedUserCode },
        });

        if (existingUserWithCode) {
          return NextResponse.json(
            { success: false, error: `User code ${providedUserCode} is already in use. Please choose a different code.` },
            { status: 400 }
          );
        }
        ...
        try {
          ...
          return NextResponse.json(
            { success: false, error: 'Failed to generate user code. No available codes.' },
            { status: 500 }
          );
        }
      }
    ...
    return NextResponse.json({
      ...
      userCode, // Return the generated user code to show to admin
    });
  } catch (error) {
