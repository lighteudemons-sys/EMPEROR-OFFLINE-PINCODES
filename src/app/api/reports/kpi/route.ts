...
export async function GET(request: NextRequest) {
  try {
    ...
    const extractVariantMultiplier = (item: any): number => {
      // First, check if customVariantValue is already stored (for new orders)
      if (item.customVariantValue && item.customVariantValue > 0) {
        return item.customVariantValue;
      }

      // Try to extract from variantName for old orders with weight data
      if (item.variantName) {
        ...
        if (match && match[1]) {
          ...
          if (!isNaN(multiplier) && multiplier > 0) {
            console.log(`[KPI API] Extracted variant multiplier from variantName:`, {
              itemId: item.id,
      ...
      if (item.menuItem && item.unitPrice && item.unitPrice > 0 && item.menuItem.price > 0) {
        ...
        // Only use this if the ratio is less than 0.9 (suggests partial portion)
    ...
    mainOrders.forEach(order => {
      order.items.forEach(item => {
        ...
        // Returns 1 for predefined variants, the multiplier for custom weight variants
        ...
        ingredientMap.forEach((quantity, ingredientId) => {
          ...
          // Scale the recipe quantity by the variant multiplier
    ...
    // Payment method breakdown — split card subtypes into separate entries
    const paymentMethods = mainOrders.reduce((acc: any, order) => {
      const method = (order.paymentMethod || 'cash').toLowerCase();
      const detail = order.paymentMethodDetail?.toUpperCase();

      if (method === 'card' && detail) {
        // Map card subtypes to individual payment method keys
        const key = detail === 'INSTAPAY' ? 'instapay'
          : detail === 'MOBILE_WALLET' ? 'mobile_wallet'
          : detail === 'TALABAT' ? 'talabat'
          : 'card';
        if (!acc[key]) {
          acc[key] = { count: 0, revenue: 0 };
        }
        acc[key].count += 1;
        acc[key].revenue += order.subtotal;
      } else if (method === 'credit') {
        if (!acc['credit']) {
          acc['credit'] = { count: 0, revenue: 0 };
        }
        acc['credit'].count += 1;
        acc['credit'].revenue += order.subtotal;
      } else {
        const key = method === 'cash' ? 'cash' : method;
        if (!acc[key]) {
          acc[key] = { count: 0, revenue: 0 };
        }
        acc[key].count += 1;
