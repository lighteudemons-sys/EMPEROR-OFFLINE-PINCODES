...
export async function POST(request: NextRequest) {
  try {
    ...
    const { branchName, licenseKey, licenseExpiresAt, phone, address, shiftReportEmail } = body;
    ...
    const branch = await db.branch.create({
      data: {
        ...
        shiftReportEmail: shiftReportEmail || null,
        isActive: true,
      },
    });
...
export async function PATCH(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, branchName, licenseKey, licenseExpiresAt, isActive, phone, address, shiftReportEmail } = body;
    ...
    const branch = await db.branch.update({
      ...
      data: {
        ...
        ...(shiftReportEmail !== undefined && { shiftReportEmail: shiftReportEmail || null }),
      },
    });
