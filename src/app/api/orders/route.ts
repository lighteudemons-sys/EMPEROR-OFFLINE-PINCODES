  ...
  try {
    ...
    for (const item of items) {
      ...
      try {
        ...
        console.error('[Order] Error fetching menu item:', {
          menuItemId: item.menuItemId,
          menuItemIdType: typeof item.menuItemId,
        ...
        return NextResponse.json(
          {
            ...
            details: `Menu item ID "${item.menuItemId}" is not valid. Please clear your cart and try again.`
          },
          { status: 400 }
        );
      }
      ...
      if (!menuItem) {
        console.error('[Order] Menu item not found:', {
          menuItemId: item.menuItemId,
          allItemsInCart: items.map(i => ({ id: i.menuItemId })),
        });
        return NextResponse.json(
          {
            success: false,
            error: 'Menu item not found',
            details: `Menu item with ID "${item.menuItemId}" no longer exists. Please clear your cart and try again.`
          },
          { status: 404 }
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            details: `Menu item "${menuItem.name}" is currently inactive.`
          },
          { status: 400 }
        );
      }
      ...
      orderItemsToCreate.push({
        ...
        requiresCaptainReceipt: menuItem.categoryRel?.requiresCaptainReceipt || false,
      });
    ...
    const totalAmount = calculateTotalAmount(subtotal, taxAmount, deliveryFee || 0, loyaltyDiscount || 0, promoDiscount || 0, manualDiscountAmount || 0);
    ...
    // Initialize ETA fields if configured for offline support
    let etaFields = {};
    const etaSettings = await db.branchETASettings.findUnique({
      where: { branchId },
    });

    if (etaSettings && etaSettings.isActive) {
      // Generate UUID locally for offline support
      const { generateDocumentUUID } = await import('@/lib/eta/ubl-generator');
      const etaUUID = generateDocumentUUID();

      etaFields = {
        etaUUID,
        etaSubmissionStatus: 'PENDING',
        etaDocumentType: '389', // 389 = Receipt
        etaSettingsId: etaSettings.id,
      };

      console.log(`[Order] ETA initialized for offline order ${finalOrderNumber}`, {
        etaUUID,
        branchId,
    ...
    const order = await db.$transaction(async (tx) => {
      ...
      const newOrder = await tx.order.create({
        data: {
          ...
          // ETA fields for offline support
          ...etaFields,
        },
      });
    ...
    return NextResponse.json({
      ...
      order: {
        ...
        manualDiscountComment: responseOrder.manualDiscountComment,
        cardReferenceNumber: cardReferenceNumber || null,
        items: order.items.map(item => ({
          ...
          requiresCaptainReceipt: item.requiresCaptainReceipt,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filterPaymentMethodDetail = searchParams.get('paymentMethodDetail');
    ...
    // Server-side payment method filter (for Reports breakdown)
    if (filterPaymentMethod) {
      where.paymentMethod = filterPaymentMethod;
    }
    if (filterPaymentMethodDetail) {
      where.paymentMethodDetail = filterPaymentMethodDetail;
    }
