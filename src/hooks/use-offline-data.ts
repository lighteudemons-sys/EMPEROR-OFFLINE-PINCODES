    ...
    try {
      if (isActuallyOffline || shouldSkipAPI) {
        ...
        if (response.ok) {
          ...
          } else if (apiEndpoint && apiEndpoint.includes('/api/ingredients')) {
            indexedDBStorage.batchSaveIngredients(Array.isArray(data) ? data : (data.ingredients || [])).catch(e =>
              console.log('[useOfflineData] Failed to cache ingredients:', e.message)
            );
          }
        } else {
